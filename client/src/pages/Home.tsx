...
export default function Home() {
  ...
  function publish() {
    if (!content.trim() || content.includes("Paste a note")) {
      toast.error("Add a note before creating a drop.", { description: "Your content stays local until you publish." });
      return;
    }
    setPublished(true);
    toast.success("Drop created", { description: "Your protected link is ready to share." });
  }

  function copyLink() {
    navigator.clipboard?.writeText("https://securedrop.local/d/7K4-M9Q");
        ...
        <div className="workspace-content">
          ...
          <section className="composer-grid">
            ...
            <aside className={`protection-card ${showProtection ? "expanded" : ""}`}>
              ...
              <div className="control-group"><label>Self-destruct</label><div className="select-wrap"><select value={expiry} onChange={(e) => setExpiry(e.target.value)}><option>Burn after reading</option><option>In 1 hour</option><option>In 24 hours</option><option>In 7 days</option><option>Never</option></select><ChevronDown size={15} /></div></div>
              <div className="toggle-row"><div><strong>Require a passphrase</strong><span>Extra layer for the link</span></div><button className={`switch ${password ? "on" : ""}`} role="switch" aria-checked={password} onClick={() => setPassword(!password)}><span /></button></div>
          ...
          <section className="recent-section"><div className="section-heading"><div><div className="object-label"><Clock3 size={14} /> RECENTLY HANDLED</div><h2>Your drops</h2></div><button className="text-button">Open the ledger <ArrowUpRight size={15} /></button></div><div className="artifact-note"><div className="artifact-thumb" style={{ backgroundImage: `url(${paperImage})` }}><span>DROP<br />INDEX</span></div><div><div className="object-label">THE PAPER TRAIL / YOUR LIFECYCLE</div><p>Every note has a boundary. Track what is waiting, what was opened, and what is about to disappear.</p></div><ArrowUpRight size={17} /></div><div className="recent-grid">{recentDrops.map((drop) => <button className="drop-row" key={drop.title} aria-label={`Open ${drop.title}; ${drop.meta}`}><div className="drop-icon"><FileLock2 size={17} /></div><div className="drop-info"><strong>{drop.title}</strong><span>{drop.meta}</span></div><span className={`drop-tag ${drop.tone}`}>{drop.tag}</span><ArrowUpRight className="row-arrow" size={16} /></button>)}</div></section>
